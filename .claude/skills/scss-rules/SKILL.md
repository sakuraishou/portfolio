...
- **実装は `md`（768〜）に統一する**。`lg` 等の中間段階は原則使わない（過去の `lg` 指定は `md` に集約済み）。どうしても段階が要るときだけ map の他 BP を使う。
...
- フォントは mixin / 変数で：本文・日本語は `$font-main`（Zen Kaku）、見出し英字は `@include display`（Fraunces）、ラベル・データの英字（等幅）は `@include mono`（IBM Plex Mono）。※旧 `@include quicksand`（Quicksand）は廃止。
