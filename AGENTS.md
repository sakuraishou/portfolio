...
0. **言語**: このプロジェクトでは **基本的に日本語** を用いる（ユーザー向け文言、コメント、ドキュメント、**Git のコミットメッセージ・PR 説明**、Cursor 上の説明・提案など）。識別子・ライブラリに合わせる名前は英語のままでよい。詳細は `.cursor/rules/project-language.mdc` を参照。コミットテンプレートを使う場合はリポジトリの `.gitmessage` を参照。
